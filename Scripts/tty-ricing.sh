...
echo -e "\e[1;36m>>> INITIALIZING FANCY TTY MODE (KMSCON + MOUSE + THEMES)...\e[0m"

# --- 1. DEPENDENCY CHECKS ---
if ! command -v kmscon &> /dev/null; then
    echo "==============================================================="
    echo "❌ CRITICAL MISSING COMPONENT: kmscon"
    echo "---------------------------------------------------------------"
    echo "Please run this as your NORMAL user first:"
    echo "   yay -S kmscon"
    echo "==============================================================="
    exit 1
fi
...
# We remove 'gpm' from the install list if it's there, or ensure we don't enable it later
...
# --- 7. RESOLUTION LOGIC ---
echo -e "\n\e[1;33m>>> RESOLUTION SELECTOR <<<\e[0m"
...
if [ "$LOADER_FOUND" = false ]; then
    echo "⚠️ No standard bootloader found. Manually add: $VIDEO_ARG $GPU_ARGS"
fi
...
echo "=========================================="
echo "    FANCY TTY MODE COMPLETE: REBOOT NOW."
echo "=========================================="
